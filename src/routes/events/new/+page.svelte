<script lang="ts">
  ...
  function setLL(longitude: number, latitude: number) {
    if (marker) {
      marker.remove();
    }
    if (map) {
...
<div class="grid grid-cols-12 space-x-2">
  <div class="map col-span-8 w-[50rem] h-[30rem]" bind:this={mapElement}></div>
  <div class="col-span-4 space-y-2 w-[25rem] h-[30rem]">
    ...
    <div class="overflow-y-auto h-full space-y-2">
      {#if searchResults}
        {#each searchResults as searchResult}
          <div
            class="bg-white rounded p-2 grid grid-cols-6 space-y-1 items-center"
          >
            <div class="col-span-4">
              <p class="text-lg">{searchResult.name}</p>
              <p class="text-base">{searchResult.address}</p>
            </div>
            <button
              class="col-span-2 bg-blue-500 p-y-2 rounded-lg w-24 h-12"
              on:click={() =>
                setLL(searchResult.longitude, searchResult.latitude)}
              >Auswählen</button
            >
          </div>
        {/each}
